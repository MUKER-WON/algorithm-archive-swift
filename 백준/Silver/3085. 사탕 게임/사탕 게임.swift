...
for y in 0..<N {
    ans = max(ans, [check(G[y]), check((0..<N).map { G[y][$0] })].max()!)
    for x in 0..<N {
        let (cY,cX) = (y+1,x+1)
        if cY < N && G[y][x] != G[cY][x] {
            var G = G
            (G[y][x],G[cY][x]) = (G[cY][x],G[y][x])
            ans = max(ans, [check(G[y]), check(G[cY]), check((0..<N).map { G[$0][x] })].max()!)
        }
        if cX < N && G[y][x] != G[y][cX] {
            var G = G
            (G[y][x],G[y][cX]) = (G[y][cX],G[y][x])
            ans = max(ans, [check(G[y]), check((0..<N).map { G[$0][cX] }), check((0..<N).map { G[$0][x] })].max()!)
        }
    }
}
print(ans)